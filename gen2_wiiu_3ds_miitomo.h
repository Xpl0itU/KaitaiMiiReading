#ifndef GEN2_WIIU_3DS_MIITOMO_H_
#define GEN2_WIIU_3DS_MIITOMO_H_
...
public:
    ...
    gen2_wiiu_3ds_miitomo_t(kaitai::kstream* p__io, kaitai::kstruct* p__parent = 0, gen2_wiiu_3ds_miitomo_t* p__root = 0);
...
private:
    ...
    std::vector<uint8_t>* m_system_id;
    std::vector<uint8_t>* m_avatar_id;
...
public:
     ...
     * Always 3?
     */
    uint8_t unknown_1() const { return m_unknown_1; }

    /**
     * 0=JPN+USA+EUR, 1=CHN, 2=KOR, 3=TWN
     */
    uint64_t character_set() const { return m_character_set; }

    /**
     * 0=no lock, 1=JPN, 2=USA, 3=EUR
     */
    uint64_t region_lock() const { return m_region_lock; }
    bool profanity_flag() const { return m_profanity_flag; }
    bool copying() const { return m_copying; }
    uint64_t unknown_2() const { return m_unknown_2; }
    uint64_t mii_position_slot_index() const { return m_mii_position_slot_index; }
    uint64_t mii_position_page_index() const { return m_mii_position_page_index; }
    uint64_t version() const { return m_version; }
    uint64_t unknown_3() const { return m_unknown_3; }
    std::vector<uint8_t>* system_id() const { return m_system_id; }
    std::vector<uint8_t>* avatar_id() const { return m_avatar_id; }
    std::vector<uint8_t>* client_id() const { return m_client_id; }
    uint16_t padding() const { return m_padding; }
    uint16_t data_1() const { return m_data_1; }
    std::string mii_name() const { return m_mii_name; }
    uint8_t body_height() const { return m_body_height; }
    uint8_t body_weight() const { return m_body_weight; }
    uint64_t face_color() const { return m_face_color; }
    uint64_t face_type() const { return m_face_type; }
    bool mingle() const { return m_mingle; }
    uint64_t face_makeup() const { return m_face_makeup; }
    uint64_t face_wrinkles() const { return m_face_wrinkles; }
    uint8_t hair_type() const { return m_hair_type; }
    uint64_t unknown_5() const { return m_unknown_5; }
    bool hair_flip() const { return m_hair_flip; }
    uint64_t hair_color() const { return m_hair_color; }
    uint32_t eye() const { return m_eye; }
    uint32_t eyebrow() const { return m_eyebrow; }
    uint16_t nose() const { return m_nose; }
    uint16_t mouth() const { return m_mouth; }
    uint16_t mouth2() const { return m_mouth2; }
    uint16_t beard() const { return m_beard; }
    uint16_t glasses() const { return m_glasses; }
    uint16_t mole() const { return m_mole; }
    ...
    gen2_wiiu_3ds_miitomo_t* _root() const { return m__root; }
    kaitai::kstruct* _parent() const { return m__parent; }
};

#endif  // GEN2_WIIU_3DS_MIITOMO_H_
